...
- 截图模式分离：默认使用实时截图，只有开启共享截图目录后才会读取 Docker/挂载目录内的截图
...
- Docker 等无法直接访问桌面的环境：开启 `use_shared_screenshot_dir`，并设置 `shared_screenshot_dir`
- 开启共享截图目录后，插件会优先读取该目录中的最新截图；如果插件本身也能截图，还会自动更新目录内的 `screenshot_<时间戳>.jpg` 和 `screenshot_latest.jpg`
...
- 截图来源默认恢复为实时截图，共享截图目录改为 Docker 场景下的显式开关
- 版本信息已统一更新为 `2.4.1`
